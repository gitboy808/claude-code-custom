...
カスタム Claude Code ステータスラインを生成・インストール — 表示要素、カラーテーマ、プレフィックスアイコンを選択。

表示内容（モデル名、コンテキストバー、出力スタイル、Git ブランチ、ディレクトリ、Vim モード）、テーマ（Gruvbox Dark、Dracula、Robbyrussell、Minimal）、出力スタイル用プレフィックスアイコンを選択できます。
...
# 自然言語
/webup-statusline レトロテーマ ブランチとコンテキストバー付き
```
...
| テーマ | スタイル |
|--------|----------|
| 🌾 Gruvbox Dark | レトロ暖色 — ティール、アクア、イエロー、グリーン |
| 🧛 Dracula | モダンダーク — パープル、ピンク、シアン |
| 💎 Robbyrussell | クラシック oh-my-zsh — 赤ディレクトリ、緑矢印 |
| 🪶 Minimal | クリーン — 装飾なし、ディムセパレーターのみ |
...
> ⚠️ **注意：** 生成されたステータスラインスクリプトは JSON 解析に `jq` が必要です。
