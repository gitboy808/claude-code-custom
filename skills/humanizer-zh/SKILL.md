...
1. 判断文本类型和主线。
   公告、说明文、技术文档优先准确和克制；博客、专栏、书稿、评论、产品分析可以更有节奏和作者判断。动笔前先能用一句话说出原文在回答什么问题；说不出就先问，不要直接改。
2. 对照 `## Core Rules` 逐节检查全文，列出所有命中位置。
   每一处都要有处理：改、删、并或挪。这份清单列完之前不动笔。
3. 决定改写力度。
   轻度润色只清理措辞和标点；深度改写要重排句子顺序、合并弱句、补足主语或因果关系，并按 `## Deep Review` 加载对应 references。
4. 完成改写后，逐项过 `## Final Check`。
...
询问时，先读 [references/voices/index.md](references/voices/index.md)，把 8 位作者的一句话简介列表贴给用户。
...
- 正文引号默认用全角双引号 `""`，嵌套用全角单引号 `''`；使用 `「」` 时嵌套用 `『』`。两种样式全篇只选其一；什么时候改用 `「」`，见 `## Repo Overrides`。
...
如果本会话已选定作者声音，确认对应的 [references/voices/&lt;author&gt;.md](references/voices/) 已加载（选用和优先级规则见 `## Voice Adoption`）。
