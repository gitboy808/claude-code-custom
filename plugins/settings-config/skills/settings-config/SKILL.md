...
3. **执行脚本**
   - install / adjust：
     ```bash
     npx -y bun ${SKILL_DIR}/scripts/configure.mjs --elements <keys> --<key1> <v1> --<key2> <v2> ... --install
     ```
   - restore：
     ```bash
...
npx -y bun ${SKILL_DIR}/scripts/configure.mjs --elements language,effortLevel,permissions.mode --language English --install
